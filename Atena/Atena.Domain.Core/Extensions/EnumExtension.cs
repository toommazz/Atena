    ...
    public static class EnumExtension
    {
        ...
        public static string GetDescription<T>(this Enum @enum)
        {
            ...
            for (var i = 0; i < enumeratorItems.Length; i++)
            {
                ...
                var attributes = (DescriptionAttribute[])fieldInfo.GetCustomAttributes(typeof(DescriptionAttribute), false);
                description[i] = (attributes.Length > 0) ? attributes[0].Description : enumeratorItems[i]?.ToString();
            }
        ...
        public static T GetEnumerator<T>(string description)
        {
            ...
            throw new InvalidOperationException(string.Format("Enumerator {0} with description {1} not found", type, description));
        }
